...
func NewMusicWorker(bot *discordgo.Session, activeGuild *services.ActiveGuild, guildId, channelId string) error {
	...
	for media := range activeGuild.MediaChan {
		if !voice.Ready {
			...
			zlog.Info().Msg(fmt.Sprintf("[%s] VoiceConnection no longer in ready state, reconnecting", activeGuild.Name))
			voice, err = bot.ChannelVoiceJoin(guildId, channelId, false, true)
			if err != nil {
				return err
			}
...
func cleanUpGuildWorker(activeGuild *services.ActiveGuild) {
...
func play(voice *discordgo.VoiceConnection, media *services.Media, activeGuild *services.ActiveGuild) {
